...
打开终端，输入`tmux -2`运行tmux(-2表示用256色运行，视觉效果要好些)，然后输入`Ctrl+A,$`修改窗口的名字，比如`testwindow`，然后`Ctrl+A,d`将当前窗口detach。

detach的窗口可以通过
```bash
$ tmux list-sessions
```
查看，并且可以通过(以窗口名字为`testwindow`为例)
```bash
$ tmux at -t testwindow
```
将其切换到前台(非常有用!)
...
打开路由器，查看网络运营商分配的ip地址，ssh默认的端口号是22，路由器在接收到端口号22的请求时不知道如何处理这个程序，进入路由器的配置界面(通常是192.168.1.1)，选择`转发规则`选项卡，打开`虚拟服务器`项->添加新条目
...
6. 如果服务器的ip地址频繁变更，可以使用DDNS-动态DNS 服务来实现ssh访问，参考:[使用DDNS+SSH连通家庭/工作电脑](http://blog.log4d.com/2011/06/ddns-ssh/)
